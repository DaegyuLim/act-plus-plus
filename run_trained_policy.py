...
def main(args):
    ...
    image_recorder = ImageRecorder(
        camera_names=task_config["camera_names"], init_node=False
    )
    dsr = drlControl(robot_id_list=robot_id_list, hz=HZ, init_node=True, teleop=False)
    gripper = gripperControl(
        robot_id_list=robot_id_list, hz=HZ, init_node=False, teleop=False
    )
    ...
    if relative_obs_mode:
        ...
        relative_robot_obs_history = np.zeros(
            (num_robot_obs, state_dim), dtype=np.float32
        )
    else:
        robot_obs_history = np.zeros((num_robot_obs, state_dim), dtype=np.float32)
    all_time_actions = np.zeros(
        (max_timesteps, max_timesteps + num_queries, action_dim), dtype=np.float32
    )
    ...
    if use_rotm6d is True:
        ...
        for r in range(num_robots):
            dsr_state_rotation = Rotation.from_euler(
                "ZYZ", dsr_state_euler[3 * r : 3 * r + 3], degrees=True
            )
            dsr_state_rotm = dsr_state_rotation.as_matrix()
            dsr_state_rotm6d = np.concatenate(
                (dsr_state_rotm6d, dsr_state_rotm[:, 0], dsr_state_rotm[:, 1]), axis=-1
            )
        ...
        if use_inter_gripper_proprio_input:
            ...
            rel_xpos[:] = reference_ee_rotm.transpose().dot(
                dsr_state_xpos[3:6] - reference_ee_pos
            )
            right_hand_rotation = Rotation.from_euler(
                "ZYZ", dsr_state_euler[3:6], degrees=True
            )
            rel_rotm_temp = (
                reference_ee_rotm.transpose() * right_hand_rotation.as_matrix()
            )
            rel_rotm6d[:] = np.concatenate(
                (rel_rotm_temp[:, 0], rel_rotm_temp[:, 1]), axis=-1
            )

            robot_state = np.concatenate(
                [dsr_state_xpos, dsr_state_rotm6d, rel_xpos, rel_rotm6d, gripper_state],
                axis=-1,
            )
        else:
            # initialize
            robot_state = np.concatenate(
                [dsr_state_xpos, dsr_state_rotm6d, gripper_state], axis=-1
            )
    else:
        robot_state = np.concatenate(
            [dsr_state_xpos, dsr_state_euler, gripper_state], axis=-1
        )

    # robot_state = pre_process(robot_state)
    robot_obs_history[:] = robot_state

    image_sampling = range(
        0, (num_image_obs - 1) * image_obs_every + 1, image_obs_every
    )
    for cam_name in camera_names:
        ...
        image_obs_history[cam_name] = np.repeat(
            first_image[np.newaxis, :, :, :],
            (num_image_obs - 1) * image_obs_every + 1,
            axis=0,
        )  # 0xxx0xxx0
        if use_depth:
            first_depth = np.array(
                [
                    image_recorder.get_depth_images()[cam_name],
                    image_recorder.get_depth_images()[cam_name],
                    image_recorder.get_depth_images()[cam_name],
                ]
            )
            depth_obs_history[cam_name] = np.repeat(
    ...
    for t in tqdm(range(max_timesteps)):
        ...
        if True:
            ...
            with torch.inference_mode():
                ...
                if relative_obs_mode:
                    ...
                    for t_obs in range(num_robot_obs):
                        ...
                        robot_rotation = Rotation.from_euler(
                            "ZYZ", robot_obs_history[t_obs + 1, 3:6], degrees=True
                        )
                        robot_rotm = (
                        ...
                        relative_robot_obs_history[t_obs, 3:6] = (
                            rel_robot_rotation.as_rotvec()
                        )
                        # position
                        relative_robot_obs_history[t_obs, 0:3] = (
                            reference_state_rotation.as_matrix().transpose()
                        ).dot(robot_obs_history[t_obs + 1, 0:3] - reference_state_pos)
                    # pass gripper position
                    relative_robot_obs_history[:, 6] = robot_obs_history[:-1, 6]
                    print(
                        "relative_robot_obs_history pre: \n", relative_robot_obs_history
                    )
                ...
                for cam_name in camera_names:
                    ...
                    if num_image_obs > 1:
                        image_obs_history[cam_name][1:] = image_obs_history[cam_name][
                            :-1
                        ]
                ...
                if use_depth:
                    ...
                    if num_image_obs > 1:
                        depth_obs_history[cam_name][1:] = depth_obs_history[cam_name][
                            :-1
                        ]
                ...
                for cam_name in camera_names:
                    all_cam_images.append(
                        np.array(image_obs_history[cam_name])[image_sampling]
                    )

                if use_depth:
                    for cam_name in camera_names:
                        all_cam_images.append(
                            np.array(depth_obs_history[cam_name])[image_sampling]
                        )
                ...
                # move data into GPU
                if use_gpu_for_inference:
                    robot_obs_history_flat = (
                        torch.from_numpy(robot_obs_history_flat)
                        .float()
                        .cuda()
                        .unsqueeze(0)
                    )
                    cam_images = (
                        torch.from_numpy(all_cam_images / 255.0)
                        .float()
                        .cuda()
                        .unsqueeze(0)
                    )
                ...
                # policy inference
                all_actions = policy(
                    robot_obs_history_flat, cam_images
                )  # action dim: [1, chunk_size, action_dim]
                ...
                if relative_action_mode:
                    dsr_rotation = Rotation.from_euler(
                        "ZYZ", dsr_state_euler, degrees=True
                    )
                    dsr_state_rotm = dsr_rotation.as_matrix()
                    rel_all_actions = all_actions

                    rel_all_actions_rotation = Rotation.from_rotvec(
                        rel_all_actions[:, 3:6]
                    )
                    all_actions_rotm = (
                        dsr_state_rotm * rel_all_actions_rotation.as_matrix()[:]
                    )
                    all_actions_rotation = Rotation.from_matrix(all_actions_rotm)

                    for i in range(num_queries):
                        # position
                        all_actions[i, 0:3] = dsr_state_xpos + dsr_state_rotm.dot(
                            rel_all_actions[i, 0:3]
                        )
                        # euler
                        all_actions[i, 3:6] = all_actions_rotation[i].as_euler(
                            "ZYZ", degrees=True
                        )
                ...
                if temporal_ensemble:
                    ...
                    exp_weights = np.expand_dims(
                        exp_weights, axis=1
                    ).transpose()  # (1, chunk_size)

                    # print(actions_for_curr_step.shape)
                    # print(exp_weights.shape)
                    action = np.sum(
                        exp_weights.dot(actions_for_curr_step), axis=0, keepdims=False
                    )
                else:
        ...
        if use_rotm6d is True:
            ...
            for r in range(num_robots):
                action_rotm6d = action[
                    3 * num_robots + 6 * r : 3 * num_robots + 6 * (r + 1)
                ]
                action_rotm_x_axis = action_rotm6d[0:3] / np.linalg.norm(
                    action_rotm6d[0:3]
                )
                action_rotm_y_axis = action_rotm6d[3:6]
                action_rotm_y_axis = (
                    action_rotm_y_axis
                    - np.dot(action_rotm_y_axis, action_rotm_x_axis)
                    * action_rotm_x_axis
                )
                action_rotm_y_axis = action_rotm_y_axis / np.linalg.norm(
                    action_rotm_y_axis
                )
                action_rotm_z_axis = np.cross(action_rotm_x_axis, action_rotm_y_axis)
                action_rotm_z_axis = action_rotm_z_axis / np.linalg.norm(
                    action_rotm_z_axis
                )
                action_rotm = np.concatenate(
                    [
                        action_rotm_x_axis[:, None],
                        action_rotm_y_axis[:, None],
                        action_rotm_z_axis[:, None],
                    ],
                    axis=1,
                )
                action_rotation = Rotation.from_matrix(action_rotm)
                action_euler = np.concatenate(
                    [action_euler, action_rotation.as_euler("ZYZ", degrees=True)],
                    axis=-1,
                )

            dsr_desired_pose = np.zeros(0)
            for r in range(num_robots):
                dsr_desired_pose = np.concatenate(
                    [
                        dsr_desired_pose,
                        action[3 * r : 3 * r + 3],
                        action_euler[3 * r : 3 * r + 3],
                    ]
                )
        else:
        ...
        if record_data:
            ...
            for cam_name in camera_names:
                data_dict[f"/observations/images/{cam_name}"].append(
                    (image_recorder.get_images())[cam_name]
                )
                data_dict[f"/observations/depth_images/{cam_name}"].append(
                    (image_recorder.get_depth_images())[cam_name]
                )
        # t2 = time.time() #
        ...
        if COMPRESS:
            ...
            for cam_name in camera_names:
                ...
                for depth in depth_list:
                    result, encoded_depth = cv2.imencode(
                        ".jpg", depth, encode_param
                    )  # 0.02 sec # cv2.imdecode(encoded_image, 1)
            ...
            for cam_name in camera_names:
                compressed_depth_list = data_dict[
                    f"/observations/depth_images/{cam_name}"
                ]
        ...
        with h5py.File(dataset_path + ".hdf5", "w", rdcc_nbytes=1024**2 * 2) as root:
            ...
            for cam_name in camera_names:
                if COMPRESS:
                    ...
                    _ = image.create_dataset(
                        ...
                        (
                            max_timesteps,
                            images_size[cam_name][0],
                            images_size[cam_name][1],
                            images_size[cam_name][2],
                        ),
                        dtype="uint8",
                        chunks=(
                            1,
            ...
            if COMPRESS:
                _ = root.create_dataset(
                    "compressed_image_len", (len(camera_names), max_timesteps)
                )
                root["/compressed_image_len"][...] = compressed_image_len
                if use_depth:
                    _ = root.create_dataset(
                        "compressed_depth_len", (len(camera_names), max_timesteps)
                    )
